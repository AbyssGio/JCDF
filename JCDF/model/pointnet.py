...
def index_points(points, idx):
    ...
    repeat_shape[0] = 1
    batch_indices = jt.arange(B, dtype=jt.int64).view(view_shape).repeat(repeat_shape)
...
def farthest_point_sample(pt, npoint):
    ...
    device = xyz.device
    B, N, C = xyz.shape
    centroids = jt.zeros((B, npoint), dtype="int")
    distance = jt.ones(B, N) * 1e10
    farthest = jt.randint(0, N, (B,), dtype="int")
    batch_indices = jt.arange(B, dtype=jt.int64)
    for i in range(npoint):
        centroids[:, i] = farthest
        centroid = xyz[batch_indices, farthest, :].view(B, 1, 3)
        dist = jt.sum((xyz - centroid) ** 2, -1)
        mask = dist < jt.array(distance)
        distance[mask] = dist[mask]
        farthest = jt.max(distance, -1)[1]
...
class STN3d(nn.Module):
    def __init__(self, channel):
        ...
        self.conv1 = jt.nn.Conv1d(channel, 64, 1)
        self.conv2 = jt.nn.Conv1d(64, 128, 1)
        self.conv3 = jt.nn.Conv1d(128, 1024, 1)
    ...
    def execute(self, x):
        ...
        iden = jt.array(np.array([1, 0, 0, 0, 1, 0, 0, 0, 1]).astype(np.float32)).view(1, 9).repeat(
            batchsize, 1)
        if x.is_cuda:
            iden = iden.cuda()
        x = x + iden
...
class STNkd(nn.Module):
    def __init__(self, k=64):
        ...
        self.conv1 = jt.nn.Conv1d(k, 64, 1)
        self.conv2 = jt.nn.Conv1d(64, 128, 1)
        self.conv3 = jt.nn.Conv1d(128, 512, 1)
    ...
    def execute(self, x):
        ...
        x = nn.relu(self.bn2(self.conv2(x)))
        x = nn.relu(self.bn3(self.conv3(x)))
        x = jt.max(Var=x, dim=2, keepdim=True)[0]
        x = x.view(-1, 512)

        x = nn.relu(self.bn4(self.fc1(x)))
        x = self.fc2(x)

        iden = jt.array(np.eye(self.k).flatten().astype(np.float32)).view(1, self.k * self.k).repeat(
            batchsize, 1)
        if x.is_cuda:
            iden = iden.cuda()
        x = x + iden
...
class PointNetEncoder(nn.Module):
    def __init__(self, feature_transform=False, out_channel=64):
        ...
        self.conv1 = jt.nn.Conv1d(3, self.hidden_channel, 1)
        self.conv2 = jt.nn.Conv1d(self.hidden_channel, out_channel, 1)
...
class VPointNet(nn.Module):
    ...
    def __init__(self, img_size, hidden_dim=64, out_channels=16):
        ...
        self.conv_1 = jt.nn.Conv1d(self.concentration_dim, out_channels, 1)
    ...
    def execute(self, pt):
        ...
        x = nn.relu(self.bn_1(self.conv_1(p2)))
