...
from utils import exr_handler, rgbd2pcd # Y
...
class ClearGrasp(jdata.Dataset):
    def __init__(
    ...
    def __getitem__(self, index):
        ...
        raw_depth =jt.array(raw_depth).unsqueeze(0).float()
        render_depth = jt.array(render_depth).unsqueeze(0).float()
        # resize point cloud
        xyz_img = cv2.resize(xyz_img, (self.img_w, self.img_h), interpolation=cv2.INTER_NEAREST)
        xyz_img = jt.array(xyz_img).permute(2, 0, 1).float()
        xyz_gt = cv2.resize(xyz_gt, (self.img_w, self.img_h), interpolation=cv2.INTER_NEAREST)
        xyz_gt = jt.array(xyz_gt).permute(2, 0, 1).float()  # 3xHxW
        ...
        mask = jt.array(mask).unsqueeze(0).float()
        ...
        return {
            ...
            "R_mat": jt.init.eye(3).float(),
            "t_vec": jt.zeros(3).float(),
            "forward_rgb": cur_rgb.clone(),
            "forward_color": color.clone(),
            "forward_depth": render_depth.clone(),
            "fx": jt.array(np.array(camera_intrinsics["fx"])).float(),
            "fy": jt.array(np.array(camera_intrinsics["fy"])).float(),
            "cx": jt.array(np.array(camera_intrinsics["cx"])).float(),
            "cy": jt.array(np.array(camera_intrinsics["cy"])).float(),
        }
